...
<a href="{{ route('event-types.show', $event_type->slug) }}" class="event-type-list-preview md:hover:shadow-lg flex {{ $is_published ? '' : 'unpublished'}} {{ isset($class) ? $class : '' }}">
    <div class="grow-0 shrink-0 h-24 w-24">
        @if ($event_type->image)
            <img src="{{ $event_type->image->getResizeUrl('preview_1x', true) }}" alt="{{ $event_type->name }}">
        @endif
    </div>
    <div class="event-type-list-preview__content bg-blue-100 p-4 grow shrink flex flex-col justify-center">
