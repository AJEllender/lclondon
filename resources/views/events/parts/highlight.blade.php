<a
    ...
    class="event-highlight md:hover:shadow-lg flex flex-col {{ (($alignment ?? 'left') === 'right') ? 'md:flex-row-reverse' : 'md:flex-row' }} {{ $event->isPublished() ? '' : 'unpublished'}} {{ !empty($class) ? $class : ''}}"
>
    <div class="relative grow-0 shrink-0 h-48 w-48 sm:h-72 sm:w-72 self-center md:self-start">
        ...
        @if ($event->start_at)
            @include('parts.date-tab', [
                ...
                'class' => 'event-highlight__date-tab'
            ])
        @endif
    </div>
    <div class="event-highlight__content bg-white px-4 sm:px-8 py-4 sm:py-8 grow shrink flex flex-col justify-center">
